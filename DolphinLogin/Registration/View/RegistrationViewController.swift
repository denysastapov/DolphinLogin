...
class RegistrationViewController: UIViewController, UITextFieldDelegate {
    ...
    let gradientLayer: CAGradientLayer = {
        ...
        layer.colors = [UIColor(hex: "#546bea").cgColor, UIColor(hex: "#2624e9").cgColor]
    ...
    let registerButton = ControlsFactory.makeButton(setTitle: "Register", background: .lightGray, setTitleColor: .white)
    ...
    private func updateGradientForButton() {
        let isAllFieldsValid = viewModel.isValidRegistration()
            if isAllFieldsValid {
                gradientLayer.frame = registerButton.bounds
                registerButton.layer.insertSublayer(gradientLayer, at: 0)
